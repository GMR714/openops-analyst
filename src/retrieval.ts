...
export async function selectTools(question: string, mode: 'all' | 'str' | 'str_cp', topK = 2, candidates: Tool[] = tools): Promise<Tool[]> {
  if (mode === 'all') return candidates
  const request = () => fetch('http://127.0.0.1:11434/api/embed', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ model: 'embeddinggemma', input: [question, ...candidates.map(tool => tool.description)] })
  })
  ...
  if (body.embeddings.length !== candidates.length + 1) throw new Error('Unexpected embedding count')
  const ranked = candidates.map((tool, i) => ({ tool, score: cosine(body.embeddings[0], body.embeddings[i + 1]) }))
    .sort((a, b) => b.score - a.score)
  const selected = new Set(ranked.slice(0, Math.min(candidates.length, Math.max(1, topK))).map(item => item.tool.name))
  ...
  return candidates.filter(tool => selected.has(tool.name))
}
